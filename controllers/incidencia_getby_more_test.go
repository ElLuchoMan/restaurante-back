...
func TestIncidenciaGetByDocumentAndDateSuccess(t *testing.T) {
    ...
    w := httptest.NewRecorder(); ctx := beegoCtx.NewContext(); ctx.Reset(w, r)
    c := IncidenciaController{}; c.Ctx = ctx; c.Data = make(map[interface{}]interface{})
    c.GetByDocumentAndDate()
    if w.Code != http.StatusOK { t.Fatalf("expected 200, got %d", w.Code) }
    if !strings.Contains(w.Body.String(), "Incidencias encontradas") { t.Errorf("unexpected body: %s", w.Body.String()) }
}
