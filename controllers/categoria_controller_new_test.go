...
type catMockOrm struct{ items map[int64]models.Categoria; next int64; qs catQS }
...
func (m *catMockOrm) Insert(v interface{}) (int64, error) { c := v.(*models.Categoria); c.PK_ID_CATEGORIA = m.next; m.items[m.next] = *c; m.next++; return 1, nil }
func (m *catMockOrm) Read(v interface{}, _ ...string) error { c := v.(*models.Categoria); it, ok := m.items[c.PK_ID_CATEGORIA]; if !ok { return orm.ErrNoRows }; *c = it; return nil }
func (m *catMockOrm) Update(v interface{}, _ ...string) (int64, error) { c := v.(*models.Categoria); if _, ok := m.items[c.PK_ID_CATEGORIA]; !ok { return 0, orm.ErrNoRows }; m.items[c.PK_ID_CATEGORIA] = *c; return 1, nil }
func (m *catMockOrm) Delete(v interface{}, _ ...string) (int64, error) { c := v.(*models.Categoria); if _, ok := m.items[c.PK_ID_CATEGORIA]; !ok { return 0, orm.ErrNoRows }; delete(m.items, c.PK_ID_CATEGORIA); return 1, nil }
...
func TestCategoriaController_FullCoverage(t *testing.T) {
    ...
    c.GetAll()
    if w.Code != http.StatusOK { t.Fatalf("GetAll status %d", w.Code) }
    ...
    if resp.Code != http.StatusNotFound { t.Fatalf("expected 404, got %d", resp.Code) }
    ...
    w = httptest.NewRecorder(); ctx = context.NewContext(); ctx.Reset(w, r); ctx.Input.RequestBody = []byte(`{"nombre":"Snacks"}`)
    c = &CategoriaController{}; c.Ctx = ctx; c.Data = make(map[interface{}]interface{})
    c.Put(); if w.Code != http.StatusOK { t.Fatalf("put nf %d", w.Code) }

    // Put ok
    cat := models.Categoria{NOMBRE: "Cafe"}; m.Insert(&cat)
    r = httptest.NewRequest(http.MethodPut, "/categorias?id=1", strings.NewReader(`{"nombre":"Café"}`))
    w = httptest.NewRecorder(); ctx = context.NewContext(); ctx.Reset(w, r); ctx.Input.RequestBody = []byte(`{"nombre":"Café"}`)
    c = &CategoriaController{}; c.Ctx = ctx; c.Data = make(map[interface{}]interface{})
    ...
    w = httptest.NewRecorder(); ctx = context.NewContext(); ctx.Reset(w, r)
    c = &CategoriaController{}; c.Ctx = ctx; c.Data = make(map[interface{}]interface{})
    c.GetById(); _ = json.Unmarshal(w.Body.Bytes(), &resp)
    if resp.Code != http.StatusOK { t.Fatalf("get id ok %d", resp.Code) }
