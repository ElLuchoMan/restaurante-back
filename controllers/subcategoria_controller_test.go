...
func (m *subMockOrm) Insert(v interface{}) (int64, error) {
	s := v.(*models.Subcategoria)
	s.PK_ID_SUBCATEGORIA = m.next
	m.items[m.next] = *s
	m.next++
	return 1, nil
}
func (m *subMockOrm) Read(v interface{}, _ ...string) error {
	s := v.(*models.Subcategoria)
	it, ok := m.items[s.PK_ID_SUBCATEGORIA]
	if !ok {
...
func (m *subMockOrm) Update(v interface{}, _ ...string) (int64, error) {
	s := v.(*models.Subcategoria)
	if _, ok := m.items[s.PK_ID_SUBCATEGORIA]; !ok {
		return 0, orm.ErrNoRows
	}
	m.items[s.PK_ID_SUBCATEGORIA] = *s
	return 1, nil
}
func (m *subMockOrm) Delete(v interface{}, _ ...string) (int64, error) {
	s := v.(*models.Subcategoria)
	if _, ok := m.items[s.PK_ID_SUBCATEGORIA]; !ok {
		return 0, orm.ErrNoRows
	}
	delete(m.items, s.PK_ID_SUBCATEGORIA)
	return 1, nil
}
...
func TestSubcategoriaController_FullCoverage(t *testing.T) {
	...
	ctx = context.NewContext()
	ctx.Reset(w, r)
	ctx.Input.RequestBody = []byte(`{"nombre":"Refrescos"}`)
	c = &SubcategoriaController{}
	c.Ctx = ctx
	c.Data = make(map[interface{}]interface{})
	c.Put()
...
func TestSubcategoriaController_Put_InvalidJSON(t *testing.T) {
	...
	c.Put()
	if w.Code != http.StatusBadRequest {
		t.Fatalf("put invalid json %d", w.Code)
	}
}
